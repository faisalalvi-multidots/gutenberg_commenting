                  ...
                  if ('' === currentAttrContent || currentNewContent !== currentAttrContent) {
                    ...
                    if (currentAttrContent !== oldAttrContent) {
                      ...
                      if (0 < diff.length) {
                        ...
                        let tagArray = ['strong', 'em', 'a', 'span', 's', 'code'];
                        let formatName = {
                          'strong': 'bold',
                          'em': 'italic',
                          's': 'strikethrough',
                          ...
                          for (let v = 0; v < diff.length; v++) {
                            ...
                            if (('</del' === diffCurrentLastTag || '</ins' === diffCurrentLastTag) && '>' === diffNextCloseTag) {
                              ...
                              if (1 === diffNextCloseTag.length && '' !== diffNextOfNext) {
                                let matchDelTag = diff[v][1].match(/<del id="[\d]{10,17}" data-uid="[\d]{1,15}" style="color: #008000;">$/);
                            ...
                            if (DiffMatchPatch.DIFF_EQUAL !== operation) {
                              ...
                              if ( 'core/list' === finalBlockProps.name ) {
                                ...
                                if (DiffMatchPatch.DIFF_INSERT === operation && null !== diff[v][1].match(/<li>(.*)<\/li>$/)) {
                                  ...
                                  let diffPrevFormat = diff[v - 1][1].match(/<li><(strong|em|span)>[\w|\W]{0,1}$/)[0];
                                  let diffCurrFormat = diff[v][1].match(/<li><(strong|em|span)>[\w|\W]{0,1}$/)[0];
                                  ...
                                  let diffPrevRemainingText = diff[v - 1][1].substring(diff[v - 1][1].lastIndexOf('<li>'));
                                  let diffCurrRemainingText = diff[v][1].substring(diff[v][1].lastIndexOf('<li>'));
                                  diff[v - 1][1] = diff[v - 1][1].substring(0, diff[v - 1][1].lastIndexOf('<li>'));
                                  ...
                                  let closingInsTag = diff[v + 2][1].substring(0,6);
                          ...
                          if ('core/list' === finalBlockProps.name && 4 === diff.length) {
                            if (-1 === diff[1][0] && 1 === diff[2][0] && null !== diff[2][1].match(/^<\/ins><\/li>/) && ( null !== diff[3][1].match(/^<\/ins><\/li>$/) || null !== diff[3][1].match(/^<\/ins><\/li>(.*)/) ) ) {
                              ...
                              if ( null !== diff[3][1].match(/^<\/ins><\/li>$/) ) {
                                ...
                                let diffNextInsTag = diff[3][1].substring(0, 11);
                            ...
                            if ((patternResult || matchRegex) && DiffMatchPatch.DIFF_EQUAL !== op && '' !== currentAttrContent) {
                              ...
                              if (!isFormating) {
                                for (let h = 0; h < tagArray.length; h++) {
                                  let fullTagRegex = 'a' === tagArray[h] || 'span' === tagArray[h] ? '<' + tagArray[h] + ' (.*)>(.*)<\/' + tagArray[h] + '>' : '<' + tagArray[h] + '>(.*)<\/' + tagArray[h] + '>';
                                  let fullTagFinalRegex = new RegExp(fullTagRegex);
